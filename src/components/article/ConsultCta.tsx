...
export default function ConsultCta({ locale }: Props) {
  ...
  return (
    <aside className="mt-20 bg-[var(--color-parchment)] border border-[var(--color-line)] border-t-2 border-t-[var(--color-burgundy)] overflow-hidden grid md:grid-cols-5">
      <div className="md:col-span-3 px-7 py-9 md:px-9 md:py-10">
        <p className="eyebrow text-[var(--color-burgundy)] mb-4">
          {copy.eyebrow}
        </p>
        <h3 className="font-[family-name:var(--font-cormorant)] text-3xl md:text-4xl text-[var(--color-ink)] leading-tight">
          {copy.heading}
        </h3>
        <p className="mt-5 text-lg leading-relaxed text-[var(--color-ink-muted)] font-[family-name:var(--font-lora)]">
          {copy.body}
        </p>
        <a
          href={copy.url}
      ...
      <div className="relative hidden md:block md:col-span-2 min-h-[220px] bg-[var(--color-rule)]">
        <Image
          src="/decor/cta-accent.webp"
          alt=""
          fill
